...
class PmisBcs(models.Model):
    ...
    name = fields.Char("Bach", compute="get_ordinal")
...
class PMisChildren(models.Model):
    ...
    child_of_ids=fields.Many2many(comodel_name='res.partner', relation='person_child_rel', column1='child_id', column2='person_id',
                     string='Children',
                     help="Person relation with Children")
...
class ResPartner(models.Model):
    ...
    # joining_circular_no = fields.Char("Circular No")
    circular_date = fields.Date("Circular Date")
    circular_copy = fields.Binary("Circular Copy")
    ...
    quota_doc = fields.Binary("Document")
    ...
    country_id=fields.Many2one("res.country","Country",default=lambda self: self.env['res.country'].search([('name','=','Bangladesh')]).id)
    per_country=fields.Many2one("res.country","Country",default=lambda self: self.env['res.country'].search([('name','=','Bangladesh')]).id)
    ...
    tin_copy=fields.Binary("TIN Certificate",help='Upload TIN Certificate Here.')
