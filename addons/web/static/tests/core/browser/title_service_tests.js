...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Eagle" });
    assert.strictEqual(env.services.title.current, "Eagle");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Eagle", chat: null });
    assert.strictEqual(env.services.title.current, "Eagle");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Eagle - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Eagle", action: "Import" });
    assert.strictEqual(env.services.title.current, "Eagle - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Eagle", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Eagle - Import"); // parts is a copy!
});
